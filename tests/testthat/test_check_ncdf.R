...
test_that("line", {
  ...
  checkVals <- ncdfgeom:::check_netcdf(nc)
...
test_that("errors", {
  ...
  nc_close(nc)
  nc<-nc_open(nc_file, write = TRUE)
  expect_error(ncdfgeom:::check_netcdf(nc), "only one geometry container per file supported")

  # nc_file <- write_geometry(nc_file=tempfile(), geomData = multipointData)
  # nc <- nc_open(nc_file, write = TRUE)
  # ncatt_put(nc, nc$var$lat, "cf_role", "timeseries_id")
  # expect_error(ncdfgeom:::check_netcdf(nc), 'multiple timeseries id variables were found.')

  nc_file <- write_geometry(nc_file=tempfile(), geomData = pointData)
  nc <- nc_open(nc_file, write = TRUE)
  ncatt_put(nc, 0,"Conventions", "garbage")
  expect_warning(ncdfgeom:::check_netcdf(nc), 'File does not advertise CF conventions, unexpected behavior may result.')
  ...
  # nc_close(nc)
  # nc<-nc_open(nc_file, write = TRUE)
  # expect_error(ncdfgeom:::check_netcdf(nc), "only one node_coordinates index is supported, this file has more than one.")
  #
  # lineData <- get_fixture_data("multilinestring")
  # nc_file <- write_geometry(nc_file=tempfile(), geomData = lineData)
  # nc<-nc_open(nc_file, write = TRUE)
  ...
  # nc_close(nc)
  # nc<-nc_open(nc_file, write = TRUE)
  # expect_error(ncdfgeom:::check_netcdf(nc), "only one contiquous ragged dimension index is supported, this file has more than one.")
  #
  # multipointData <- get_fixture_data("multipoint")
  # nc_file <- write_geometry(nc_file=tempfile(), geomData = multipointData)
  # nc <- nc_open(nc_file, write = TRUE)
  # ncatt_put(nc, nc$var$lat, "standard_name", "garbage")
  # expect_warning(ncdfgeom:::check_netcdf(nc), "instance dimension is being inferred based on an assumption of dimension order of the character instance_id and may not be correct.")
  #
  # nc <- nc_open("data/borked_featureType.nc")
  #
  # expect_warning(ncdfgeom:::check_netcdf(nc), "File does not have a featureType declaration, unexpected behavior may result.")
})
