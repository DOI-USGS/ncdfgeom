...
This vignette shows how to add polygon data to a NetCDF DSG TimeSeries file, read the polygon data from the NetCDF file, and write it to a shapefile format.

First, we open a NetCDF file and some geojson data containing polygons. These files are both included in "extdata" of `ncdfgeom`.
```{r libs}
...
```{r demo}
hucPolygons_nc <- ncdfgeom::write_geometry(nc_file="hucPolygons.nc",
...
```{r read, warning=F}
hucPolygons_sf <- ncdfgeom::read_geometry("hucPolygons.nc")
