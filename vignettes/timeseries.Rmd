...
`ncdfgeom` expects timeseries data to be written into a file first. See the main vignette example for a full workflow. The vignette example goes into a bit more detail on writing timeseries data. 
...
1. `global_attributes` contains standard glabal attributes found in the file. All of the variables that have one element per timeseries variable, are named the same as the NetCDF variable names so they can be accessed by name as shown below.
