...
For intensive variables, such as precipitation depth, to calculate the value for pink-9, we would do:
...
Since we are calculating a area weighted mean for an extensive variable, we 
...
Note that because there is no contribution to 9 over some parts of the polygon, 
...
pink, the weight can be used directly without area weighting contributions.
...
(result <- result |>
  ...
  summarize(
    new_val_intensive = sum( (val * w * area) ) / sum(w * area),
